 ...
 *
 * The Prime IOContext contains a single InputStream that is used to read Records. In Hadoop, each Map operation
 * consists of a single Text entry being read from an InputSplit. It is prohibitively expensive to construct a new
 * Prime IOContext and RecordReader for each record in the InputSplit.
 *
 * This special InputStream allows for Text objects (converted to byte[]) from the InputSplit to be 'appended' to the
 * InputStream. The Prime RecordReader can be instantiated once for multiple Hadoop map() operations.
 *
